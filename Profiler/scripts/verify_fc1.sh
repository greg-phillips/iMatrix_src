...
# Note: remote_cmd, remote_has_cmd provided by common_ssh.sh
...
status "Verifying FC-1 binary on target: $TARGET (port $SSH_PORT)"
...
# Start report
{
    ...
    if remote_cmd_status "test -e $FC1_PATH"; then
    ...
    FC1_PID=$(remote_cmd "pidof FC-1 2>/dev/null || pgrep -f '/usr/qk/bin/FC-1' 2>/dev/null || true")
    if [ -n "$FC1_PID" ]; then
