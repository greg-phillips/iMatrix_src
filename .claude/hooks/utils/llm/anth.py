...
# ///
"""
Anthropic LLM Script for iMatrix_Client

Uses Anthropic's Claude for generating completion messages and agent names.

Usage:
    uv run anth.py "your prompt here"   # General prompting
    uv run anth.py --completion         # Generate completion message
    uv run anth.py --agent-name         # Generate agent name
"""
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "--completion":
            ...
            if response:
                ...
                print("Error calling Anthropic API", file=sys.stderr)
    else:
        print("Usage: ./anth.py 'prompt' | --completion | --agent-name")
