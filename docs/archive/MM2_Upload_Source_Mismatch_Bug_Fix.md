# MM2 Upload Source Mismatch and GPS EVT Read Failure Fix

**Date**: 2025-12-07
**Last Updated**: 2025-12-08
**Author**: Claude Code
**Status**: Implemented and Tested
**Affects**: Memory Manager v2.8, Host Sensor Data Upload, GPS EVT Sensors
**Commit**: 435a1ba6 (Aptera_1_Clean)
...
## GPS EVT Read Failure Fix (Added 2025-12-08)

### Problem

GPS sensors (GPS_Latitude, GPS_Longitude, GPS_Altitude, GPS_Speed) were failing during upload with:
```
Failed to read all event data for: GPS_Latitude, result: 34:NO DATA, requested: 2, received: 0
```

The issue: `imx_get_new_sample_count()` reported 2 records available, but `imx_read_bulk_samples()` returned 0.

### Root Cause

The `total_records` counter was incrementing when data was written, but the sector chain could become invalid if sectors were freed/erased without properly resetting the pointers. This created a mismatch:
- `total_records` > 0 → count function returns positive count
- `ram_start_sector_id` points to freed sector → read function finds nothing

### Fix (mm2_read.c:219-243)

Added sector chain validation before returning count from `imx_get_new_sample_count()`:
...
if (!start_entry || !start_entry->in_use) {
    ...
    PRINTF("[MM2] get_new_sample_count: sensor=%s, START SECTOR INVALID\r\n", csb->name);
...
### EVT Sensor Path Fix (hal_sample.c, hal_sample_common.c)

EVT sensors (sample_rate=0) were also being incorrectly processed via the TSD sampling path. Added early exit:

**hal_sample.c**:
```c
if (csb[*active].sample_rate == 0) {
    *active += 1;  /* Move to next sensor for next call */
    return;
}
```

**hal_sample_common.c** (imx_should_save_sample):
```c
if (csb[index].sample_rate == 0) {
    return false;  /* EVT sensors handled via event path, not sampling */
}
```
...
### Upload Source Fix (mm5.txt - mm8.txt)
...
### GPS EVT Fix (mm5.txt)
After applying the GPS sector chain validation fix:
1. GPS_Latitude: Successfully read 13 records and erased after ACK
2. GPS_Longitude: Successfully read 13 records and erased after ACK
3. GPS_Altitude: Successfully read 13 records and erased after ACK
4. GPS_Speed: Successfully read 13 records and erased after ACK
5. No "Failed to read all event data" errors
6. No "START SECTOR INVALID" messages (sector chain was valid during test)
